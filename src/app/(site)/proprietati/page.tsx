    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-12 space-y-24">
      ...
      <div>
        ...
        {featuredProps.length > 0 && (
          <section className="mb-12">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-lg font-light text-white">Proprietăți Featured</h2>
        ...
        {regularProps.length > 0 && (
          <section className="mb-12">
            {featuredProps.length > 0 && <h2 className="text-lg font-light text-white mb-6">Toate Proprietățile</h2>}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {regularProps.map((p) => (
                <PropertyCard key={p.id} property={p} />
              ))}
      ...
      <div id="oportunitati">
        ...
        {featuredOpps.length > 0 && (
          <section className="mb-12">
            <div className="flex items-center gap-3 mb-6">
              <h2 className="text-xl font-light text-white">Oportunități Featured</h2>
        ...
        <div className="mt-10 rounded-2xl border border-amber-500/20 bg-gradient-to-br from-amber-500/5 to-zinc-900/50 p-8">
          <div className="max-w-2xl mx-auto text-center space-y-4">
            ...
            <h3 className="text-2xl font-light text-white">Lista completă off-market — disponibilă prin cerere</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
              ...
              <a href={brandContent.contact.whatsappText} target="_blank" rel="noopener noreferrer" className="rounded-full border border-zinc-700 px-8 py-3 text-sm text-zinc-300 hover:border-amber-500/30 hover:text-white transition-all">WhatsApp Direct</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="investments">
        <section className="space-y-6">
          <h2 className="text-2xl font-light text-white">Piețe globale analizate</h2>
          <div className="grid lg:grid-cols-2 gap-6">
            {MARKETS.map((m) => {
              const c = colorSchemes[m.color] ?? colorSchemes.amber;
              return (
                <div key={m.id} className={`rounded-3xl border bg-zinc-900/30 p-7 space-y-5 transition-all hover:bg-zinc-900/50 ${m.highlight ? "border-amber-500/30 ring-1 ring-amber-500/10" : "border-zinc-800"}`}>
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <span className="text-3xl">{m.flag}</span>
                      <div>
                        <h3 className="text-lg font-light text-white">{m.country}</h3>
                        <p className="text-xs text-zinc-500 flex items-center gap-1"><MapPin className="h-3 w-3" /> {m.city}</p>
                      </div>
                    </div>
                    <div className="text-right">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    ...
                    <div>
                      ...
                      <ul className="space-y-1.5">
                        {m.cons.map((c) => (
                          <li key={c} className="flex items-start gap-2"><span className="text-amber-500/70 mt-0.5">△</span><span className="text-zinc-500">{c}</span></li>
                        ))}
                      </ul>
                    </div>
                  </div>
                  <Link href="/contact" className="block w-full rounded-full border border-zinc-700 py-2.5 text-center text-sm text-zinc-300 hover:border-amber-500/40 hover:text-white transition-all">
        ...
        <section className="rounded-3xl border border-amber-500/20 bg-gradient-to-br from-amber-500/5 to-transparent p-10 md:p-14 text-center space-y-6 mt-16">
          ...
          <h2 className="text-2xl md:text-3xl font-light text-white">Unde să investești în 2026?</h2>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <Link href="/convenience" className="rounded-full border border-zinc-700 px-8 py-3.5 text-sm text-zinc-300 hover:text-white transition-all">Calculatoare ROI →</Link>
          </div>
        </section>
      </div>
    </div>
