      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-start gap-5">
            ...
            <div>
              ...
              <h1 className="text-2xl font-light text-white mt-2 mb-1">{dev.name}</h1>
          ...
          <div className={`rounded-3xl ${designSystem.glass} p-6 sm:p-7 space-y-4`}>
            <h2 className="text-base font-light text-white tracking-wide border-b border-zinc-900 pb-2.5">
              Despre Companie
            </h2>
          ...
          {dev.projects_list && dev.projects_list.length > 0 && (
            <div className={`rounded-3xl ${designSystem.glass} p-6 sm:p-7 space-y-4`}>
              <h2 className="text-base font-light text-white flex items-center gap-2 border-b border-zinc-900 pb-2.5">
                <Layers className="h-4.5 w-4.5 text-amber-500/85" />
                Portofoliu Proiecte Reprezentative
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {dev.projects_list.map((proj, idx) => (
                  <div key={idx} className="flex items-center gap-3 rounded-xl border border-zinc-900 bg-zinc-950/40 p-4 hover:border-zinc-800 transition-colors">
          ...
          {/* Project Gallery */}
          <div className={`rounded-3xl ${designSystem.glass} p-6 sm:p-7 space-y-4`}>
            <h2 className="text-base font-light text-white border-b border-zinc-900 pb-2.5">
        ...
        <div className="space-y-6">
          ...
          <div className={`rounded-3xl ${designSystem.glass} p-6 space-y-5`}>
            <h3 className="text-xs uppercase tracking-widest text-zinc-500 font-semibold">Specificații Dezvoltator</h3>
            <div className="space-y-3.5 text-xs">
              <div className="flex justify-between py-2 border-b border-zinc-900">
                <span className="text-zinc-500">Regiune activă</span>
                <span className="text-zinc-300 font-medium text-right">{dev.cities?.join(", ") || dev.city}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-zinc-900">
                <span className="text-zinc-500">Tipuri proprietăți</span>
                <span className="text-zinc-300 font-medium text-right">{dev.property_types?.join(", ") || "Residential"}</span>
              </div>
            ...
            {dev.website && (
              <a
                ...
                className="flex items-center justify-center gap-2 w-full rounded-xl border border-zinc-800 py-3 text-xs font-semibold text-zinc-300 hover:border-amber-500/40 hover:text-white transition-all bg-zinc-900/10"
              >
