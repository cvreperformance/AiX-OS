    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-12 space-y-10 animate-in">
      ...
      <section className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          <div key={idx} className={`p-5 rounded-2xl border border-zinc-200 bg-white/20 backdrop-blur-md text-left space-y-2 relative overflow-hidden`}>
            <div className="absolute top-0 right-0 w-24 h-24 bg-amber-500/[0.01] blur-2xl rounded-full" />
            <div className="flex justify-between items-center">
              <span className="text-[9px] uppercase tracking-wider text-zinc-400 font-mono font-semibold">{stat.label}</span>
              <stat.icon className="h-4 w-4 text-amber-500/40" />
            </div>
            <p className="text-2xl font-light text-zinc-900 font-mono">{stat.val.toLocaleString()}</p>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {activeTab === "terminal" && (
          <div className="space-y-6 text-left">
            <div className={`p-6 rounded-3xl border border-zinc-200 bg-white/85 backdrop-blur-xl relative overflow-hidden space-y-4`}>
              ...
              <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-600 font-mono">LIVE INTELLIGENCE WALL</h3>
                </div>
              ...
              <div className="space-y-3 font-mono text-xs max-h-[300px] overflow-y-auto pr-2 scrollbar-thin">
                {liveFeed.map(feed => (
                  <div key={feed.id} className="flex items-start gap-4 p-2.5 rounded-lg bg-white/40 hover:bg-zinc-100/30 border border-zinc-200/60 transition-colors">
              ...
              <div className="pt-2 border-t border-zinc-200 flex justify-between items-center text-[10px] text-zinc-400 font-mono">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
            {[
              ...
              <div key={i} className={`p-5 rounded-2xl border ${agent.color} space-y-3`}>
                <div className="flex justify-between items-center">
                  <h4 className="text-xs font-semibold text-zinc-900">{agent.name}</h4>
                  <span className="text-[9px] font-bold font-mono px-2 py-0.5 border border-zinc-200 bg-white rounded-full">{agent.status}</span>
                </div>
                <p className="text-[11px] text-zinc-400 leading-relaxed">{agent.desc}</p>
                <div className="pt-2.5 border-t border-zinc-200/60 flex justify-between items-center text-[10px] font-mono text-zinc-400">
            ...
            {user ? (
              <div className="space-y-4">
                ...
                {properties.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center justify-between p-4 rounded-2xl border border-zinc-200 bg-white/30 hover:border-zinc-300 hover:bg-zinc-100/25 transition-all group"
                  >
                    <div className="space-y-1 text-left">
                      <Link href={item.href} className="text-sm font-semibold text-zinc-900 group-hover:text-amber-400 transition-colors">
                        {item.title}
                      </Link>
                    ...
                    <div className="flex items-center gap-4">
                      <Link
                        href={item.href}
                        className="text-[10px] uppercase font-mono font-bold text-zinc-400 hover:text-zinc-900 flex items-center gap-1"
                      >
              ...
              <div className={`p-8 rounded-3xl border border-zinc-200 bg-white/75 backdrop-blur-xl text-center space-y-6 relative overflow-hidden`}>
                ...
                <div className="space-y-2">
                  <h3 className="text-base font-semibold text-zinc-900">{language === "ro" ? "Workspace Privat Securizat" : "Private Workspace Secured"}</h3>
