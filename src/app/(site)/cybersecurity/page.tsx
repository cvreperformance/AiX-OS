    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-12 space-y-20 animate-in">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        {SECURITY_STATS.map((stat) => (
          ...
          >
            ...
            <p className="text-xs font-semibold text-zinc-900">{stat.label}</p>
            <p className="text-[10px] text-zinc-400 leading-snug">{stat.sublabel}</p>
          </div>
        ))}
      </section>
      ...
      <section className="space-y-8">
        <div className="border-b border-zinc-200 pb-5">
          ...
          <h2 className="text-2xl md:text-3xl font-light text-zinc-900 mt-1">Tipuri de Fraude Detectate</h2>
          ...
          {FRAUD_TYPES.map((fraud) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <h3 className="text-sm font-semibold text-zinc-900 group-hover:text-amber-400 transition-colors">{fraud.title}</h3>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
        ...
        <div className="space-y-6">
          <div>
            ...
            <h2 className="text-2xl font-light text-zinc-900 mt-1">Straturi de Protecție</h2>
          </div>
          <div className="space-y-3">
            {PROTECTION_LAYERS.map((layer, i) => (
              <div
                key={layer.label}
                className={`flex items-start gap-4 p-4 rounded-2xl border border-zinc-200 bg-white/40 hover:border-zinc-300 hover:bg-zinc-100/20 transition-all`}
              >
                ...
                <div>
                  ...
                  <p className="text-[10.5px] text-zinc-400 mt-0.5 leading-relaxed">{layer.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className={`p-6 sm:p-8 rounded-3xl ${designSystem.glass} space-y-6 h-fit`}>
          <div className="flex items-center gap-3 border-b border-zinc-200/60 pb-5">
            <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 p-2.5 text-amber-400">
              <ShieldAlert className="h-5 w-5" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-zinc-900">Due Diligence Checklist</h3>
              <p className="text-[10px] text-zinc-400 font-mono uppercase tracking-wide mt-0.5">Standard HNWI Transaction</p>
            </div>
          </div>
      ...
      <section className="space-y-8">
        <div className="border-b border-zinc-200 pb-5">
          ...
          <h2 className="text-2xl font-light text-zinc-900 mt-1">Instrumente & Resurse Securitate</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {RESOURCES.map((r) => (
            ...
            >
              <div>
                <h3 className="text-sm font-semibold text-zinc-900 group-hover:text-amber-400 transition-colors">{r.title}</h3>
                <p className="text-xs text-zinc-450 mt-1.5 leading-relaxed">{r.desc}</p>
              </div>
              <div className="flex items-center gap-1 text-[10px] font-semibold text-zinc-400 group-hover:text-zinc-900 uppercase tracking-wider mt-4">
                Accesează
      ...
      <section>
        <div className={`p-8 sm:p-12 rounded-3xl border border-red-500/20 bg-white/70 backdrop-blur-xl text-center space-y-6 relative overflow-hidden`}>
          ...
          <div className="flex flex-wrap justify-center gap-3 pt-2">
            <Link
              href="/contact"
              className="rounded-xl bg-red-500 text-zinc-900 px-6 py-2.5 text-xs font-semibold hover:bg-red-400 transition-all shadow-md shadow-red-500/10 flex items-center gap-1.5"
            >
