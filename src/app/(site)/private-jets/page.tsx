  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-12 space-y-16 animate-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 rounded-2xl border border-zinc-800 bg-zinc-950/60 backdrop-blur-xl p-6 sm:p-8 space-y-6 shadow-xl shadow-black/40">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="space-y-1">
            <h2 className="text-xl font-light text-white">Opțiuni Flotă Recomandate</h2>
          ...
          <div className="space-y-4">
            {FLEET.map((jet) => (
              <div
                key={jet.id}
                className="group relative rounded-2xl border border-zinc-800 bg-zinc-900/20 p-5 flex flex-col md:flex-row gap-5 hover:border-zinc-700 transition-all duration-300 overflow-hidden"
              >
                {/* Visual Glow */}
                ...
                <div className="flex-1 flex flex-col justify-between space-y-3">
                  <div>
                    <div className="flex justify-between items-start">
                      <h3 className="text-base font-semibold text-white">{jet.name}</h3>
                  ...
                  {/* Jet Specs Row */}
                  <div className="grid grid-cols-3 gap-2 py-2 border-t border-b border-zinc-800/80 text-[10px]">
                    <div>
                      <p className="text-zinc-500 uppercase tracking-wider">Pasageri</p>
                      <p className="font-medium text-white">{jet.pax} Locuri</p>
                    </div>
                    <div>
                      <p className="text-zinc-500 uppercase tracking-wider">Autonomie</p>
                      <p className="font-medium text-white">{jet.range}</p>
                    </div>
                    <div>
                      <p className="text-zinc-500 uppercase tracking-wider">Viteză de croazieră</p>
                      <p className="font-medium text-white">{jet.speed}</p>
                    </div>
                  </div>

                  <div className="flex justify-between items-center text-xs">
                    <span className="text-[10px] text-zinc-500">Servicii VIP Premium Incluse</span>
                    <a
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-950/40 p-6 sm:p-8 space-y-6">
          <div className="space-y-1.5">
          ...
          <form onSubmit={handleTrack} className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-xl border border-zinc-800 bg-zinc-900/30 px-4 py-2.5 text-xs text-white placeholder-zinc-500 focus:border-amber-500 focus:outline-none transition-colors"
            />
            <button
              type="submit"
              className="rounded-xl bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-white px-5 py-2.5 text-xs font-semibold transition-colors flex items-center gap-1.5"
            >
          ...
          {trackedFlight ? (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-5 space-y-4 animate-in">
              <div className="flex justify-between items-center border-b border-zinc-800/80 pb-3">
                <div>
                  <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Aeronava</p>
                  <p className="text-xs font-semibold text-white">{trackedFlight.aircraft}</p>
                </div>
                <div className="text-right">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <div>
                    <p className="font-semibold text-white">{trackedFlight.origin}</p>
                    <p className="text-[10px] text-zinc-400">Plecare: {trackedFlight.departureTime}</p>
                  </div>
                  <div>
                    <p className="font-semibold text-white text-right">{trackedFlight.destination}</p>
                ...
                {trackedFlight.progress > 0 && (
                  <div className="space-y-1">
                    <div className="w-full bg-zinc-800 rounded-full h-1.5 relative overflow-hidden">
                      <div
              ...
              {/* Dynamic stats */}
              {trackedFlight.progress > 0 && (
                <div className="grid grid-cols-2 gap-4 pt-3 border-t border-zinc-800/80 text-[10px]">
                  <div>
                    <span className="text-zinc-500 uppercase tracking-wider block">Altitudine</span>
                    <span className="font-medium text-white">{trackedFlight.altitude}</span>
                  </div>
                  <div>
                    <span className="text-zinc-500 uppercase tracking-wider block">Viteză Sol (Ground)</span>
                    <span className="font-medium text-white">{trackedFlight.speed}</span>
                  </div>
                </div>
              )}
            </div>
          ) : (
            <div className="rounded-xl border border-zinc-800/40 bg-zinc-900/5 p-8 text-center text-zinc-500 text-xs">
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-950/40 p-6 sm:p-8 space-y-6">
          ...
          {pnrResult ? (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-5 space-y-4 animate-in">
              <div className="flex justify-between items-center border-b border-zinc-800/80 pb-3">
                <div>
                  <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Client Elitist</p>
                  <p className="text-xs font-semibold text-white">{pnrResult.client}</p>
                </div>
                <div className="text-right">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between py-1 border-b border-zinc-800/50">
                  ...
                  <span className="font-medium text-white">{pnrResult.route}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-zinc-800/50">
                  <span className="text-zinc-500">Data Plecării:</span>
                  <span className="font-medium text-white">{pnrResult.date}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-zinc-800/50">
                  <span className="text-zinc-500">Tip Aeronava:</span>
                  <span className="font-medium text-white">{pnrResult.aircraft}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-zinc-800/50">
                  <span className="text-zinc-500">VIP Terminal Lounge:</span>
                  <span className="font-medium text-white text-right">{pnrResult.loungeAccess}</span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-zinc-500">Catering Preferențial:</span>
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/20 p-6 sm:p-8 space-y-6">
        <h3 className="text-lg font-light text-white text-center">Standardele Aviației Exclusive AiX</h3>
